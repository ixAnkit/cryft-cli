...
// GetLatestAvagoByProtoVersion returns the latest cryftgo version which
...
// GetAvagoURL returns the compatibility URL for cryftgo
...
func GetVersionMapping(mapper VersionMapper) (map[string]string, error) {
	...
	// evaluate two cryftgo versions which are consecutive
	// and run with the same RPC version.
	// This is required for the for the "can deploy with multiple cryftgo versions" test
	...
	// and then a compatible cryftgo
	//
	// To avoid having to iterate again, we'll also fill the values
	// for the **latest** compatible cryftgo and Subnet-EVM
	for i, ver := range subnetEVMversions {
		...
		// there exists at least one compatible cryftgo.
	...
	// this is simpler, we just need the latest and its compatible cryftgo
	spacesVMversions, spacesVMmapping, err := getVersions(mapper, models.SpacesVM)
	if err != nil {
		return nil, err
	}
	// the assumption is that the latest SpacesVM ALWAYS has a compatible cryftgo already
...
// getAvagoCompatibility returns the compatibility for cryftgo
