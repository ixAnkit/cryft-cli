...
func (d downloader) GetLatestReleaseVersion(releaseURL string) (string, error) {
	// TODO: Question if there is a less error prone (= simpler) way to install latest cryftgo
	// Maybe the binary package manager should also allow the actual cryftgo binary for download
	token := os.Getenv(constants.GithubAPITokenEnvVarName)
